...
public class CurvedUIVertexEffect : BaseMeshEffect
{
    public override void ModifyMesh(VertexHelper vh)
    {
        ...
        if (tesselationRequired || curvingRequired || SavedVertexHelper == null ||
            SavedVertexHelper.currentVertCount == 0)
        {
    ...
    //public settings
    [Tooltip(
        "Check to skip tesselation pass on this object. CurvedUI will not create additional vertices to make this object have a smoother curve. Checking this can solve some issues if you create your own procedural mesh for this object. Default false.")]
    ...
    void Update()
    {
        ...
        // {
            ...
            if (!testedPos.AlmostEqual(savedPos))
            {
                ...
                if (mySettings.Shape != CurvedUISettings.CurvedUIShape.Bezier ||
                    Mathf.Pow(testedPos.x - savedPos.x, 2) > 0.00001 ||
                    Mathf.Pow(testedPos.z - savedPos.z, 2) > 0.00001)
                {
                    savedPos = testedPos;
    ...
    void CheckTextFontMaterial()
    {
        ...
        if (myText)
        {
            if (myText.cachedTextGenerator.verts.Count > 0)
            {
    ...
    void ModifyVerts(List<UIVertex> verts)
    {
        ...
        if (tesselatedVerts != null)
        {
            ...
            {
                copiedVerts[i] = CurveVertex(tesselatedVerts[i], angle, canvasSize);
            }
        ...
        else
        {
            ...
            {
                copiedVerts[i] = CurveVertex(verts[i], angle, canvasSize);
            }
    ...
    public int[] YangHuitriangle(int n)
    {
        int tnumber = n; //贝塞尔曲线控制点数(阶数)
        int[] mi = new int[tnumber];
        mi[0] = mi[1] = 1;
        mi[0] = mi[1] = 1;
        for (int i = 3; i <= tnumber; i++)
        {
            int[] t = new int[i - 1];
    ...
    private float[] CalculateBezierPointForCubic(List<Vector2> poss, float t)
    {
        ...
        var temp = 1 - t;
        var n = poss.Count;
        var mi = YangHuitriangle(n);
        for (int j = 0; j < 2; j++)
        {
            for (int i = 0; i < n; i++)
            {
                point[j] += poss[i][j] * Mathf.Pow(temp, n - i - 1) * Mathf.Pow(t, i) * mi[i];
            }
        }
    ...
    private float[] CalculateBezierPointForCubic(float[][] poss, float t)
    {
        var point = new float[2]
        {
            0.0f, 0.0f
    ...
    public List<Vector2> Lists1 = new List<Vector2>(5);
    public List<Vector2> Lists2 = new List<Vector2>(5);

    /// <summary>
    /// Map position of a vertex to a section of a circle. calculated in canvas's local space
    /// </summary>
    UIVertex CurveVertex(UIVertex input, float cylinder_angle, Vector2 canvasSize)
    {
        ...
        float t = ((pos.x + canvasSize.x / 2 + 130) / canvasSize.x);
        if (!isSingle)
        {
            var bezier1 = CalculateBezierPointForCubic(new float[][]
            {
                new float[] {Lists1[0].x, Lists1[0].y},
                new float[] {Lists1[1].x, Lists1[1].y},
                new float[] {Lists1[2].x, Lists1[2].y},
                new float[] {Lists1[3].x, Lists1[3].y},
                new float[] {Lists1[4].x, Lists1[4].y},
                new float[] {Lists1[5].x, Lists1[5].y},
                new float[] {Lists1[6].x, Lists1[6].y},
            }, t);
            var bezier2 = CalculateBezierPointForCubic(new float[][]
            {
                new float[] {Lists2[0].x, Lists2[0].y},
                new float[] {Lists2[1].x, Lists2[1].y},
                new float[] {Lists2[2].x, Lists2[2].y},
                new float[] {Lists2[3].x, Lists2[3].y},
                new float[] {Lists2[4].x, Lists2[4].y},
                new float[] {Lists2[5].x, Lists2[5].y},
                new float[] {Lists2[6].x, Lists2[6].y},
            }, t);
    ...
    void ModifyQuad(List<UIVertex> verts, int vertexIndex, Vector2 requiredSize)
    {
        ...
        {
            horizontalDir = (horizontalDir).x > (quad[3].position - quad[0].position).x
                ? horizontalDir
                : quad[3].position - quad[0].position;
            verticalDir = (verticalDir).y > (quad[2].position - quad[3].position).y
                ? verticalDir
        ...
        if (TransformMisaligned || mySettings.Shape == CurvedUISettings.CurvedUIShape.Bezier)
        {
