...
public static class UnityEditorUtils
{
    ...
    public static string HandleCopyPaste(int controlID)
    {
        if (controlID == GUIUtility.keyboardControl)
        {
            if (Event.current.type == UnityEngine.EventType.KeyUp && (Event.current.modifiers == EventModifiers.Control || Event.current.modifiers == EventModifiers.Command))
            {
                ...
                {
                    Event.current.Use();
                    UnityEngine.TextEditor editor = (UnityEngine.TextEditor)GUIUtility.GetStateObject(typeof(UnityEngine.TextEditor), GUIUtility.keyboardControl);
                    editor.Copy();
                }
    ...
    public static object Invoke(string className, string funcName, object obj, object[] args)
    {
        ...
        if (!funcs.TryGetValue(key, out mi))
        {
            ...
            if (t != null)
            {
                mi = t.GetMethod(funcName, BindingFlags.NonPublic | BindingFlags.Public | BindingFlags.Instance | BindingFlags.Static);
                funcs.Add(key, mi);
            }
        }
    ...
    public static void SaveTexture(Texture2D texture, string path, TextureImporterCompression comporession, bool readable, bool mipmapEnabled)
    {
    ...
    public static bool SetTextureAssetFormat(string path, TextureImporterCompression comporession, bool readable, bool mipmapEnabled)
    {
        if (string.IsNullOrEmpty(path))
        {
            return false;
    ...
    public static Texture2D CopyTexture(Texture2D tex, int destWidth, int destHeight, bool keepAlpha = true, bool forceAlpha = false)
    {
        int width = destWidth;
