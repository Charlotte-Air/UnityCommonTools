...
public class TimeUtils
{
    ...
    /// 倒计时
    /// </summary>
    /// <param name="id">返回的计时器句柄ID,用于重新调用时停止原先的计时器</param>
    ...
    public static uint GetCountDown(uint id, long time, UnityEngine.UI.Text label, Action overCallback, string sOriginalStr = "{0}:{1}:{2}", string format = "HH:MM:SS")
    {
        UIGameTimer.Instance.DelTimer(id);
        uint clockID = 0;
        ...
        Action setLable = () =>
        {
            ...
            if (label == null)
            {
                UIGameTimer.Instance.DelTimer(clockID);
                return;
            }
        ...
        else
        {
            getLeft(timeLeft);
            setLable();
            clockID = UIGameTimer.Instance.StartTimer(60, hid =>
            {
                timeLeft--;
                if (timeLeft < 0)
                {
                    UIGameTimer.Instance.DelTimer(hid);
                    if (label == null && sOriginalStr != "") return;
                    overCallback?.Invoke();
                    clockID = 0;
                    return;
