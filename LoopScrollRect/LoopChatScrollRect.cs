...
namespace UnityEngine.UI
{
    ...
    public abstract class LoopChatScrollRect : UIBehaviour, IInitializePotentialDragHandler, IBeginDragHandler, IEndDragHandler, IDragHandler, IScrollHandler, ICanvasElement, ILayoutElement, ILayoutGroup
    {
        //==========LoopScrollRect==========
        ...
        public Vector2 element_offset = new Vector2(0, 0);
        ...
        protected Vector2 m_content_anchoredpos = Vector2.zero;
        ...
        public bool isBottom
        {
            get
            {
                ...
                {
                    if (normalizedPosition.x >= 0.95f - element_offset.x && normalizedPosition.y >= 0.95f - element_offset.y)
                    {
                        m_isbottom = true;
                    }
                }
                ...
                {
                    if (normalizedPosition.y >= 0.95f - element_offset.y)
                    {
                        m_isbottom = true;
                    }
                }
                ...
                {
                    if (normalizedPosition.x >= 0.95f - element_offset.x)
                    {
                        m_isbottom = true;
                    }
                }
        ...
        public LoopScrollManager.LoopScrollCallBack m_callBack;
        public LoopScrollManager.DespawnCallBack m_despawnCallBack;
        public LoopScrollManager.SpawnCallBack m_spawnCallBack;
        public LoopScrollManager.MoveCallBack m_moveCallback;

        public Action<bool> onScrollTop;
        public Action<bool> onScrollBottom;
        public Action onScrollMove;
        public Action onScrollStop;
            ...
            if (Application.isPlaying)
            {
                ...
                for (int i = content.childCount - 1; i >= 0; i--)
                {
                    ...
                    if (m_despawnCallBack != null)
                        m_despawnCallBack(content.GetChild(i) ? content.GetChild(i) : null, itemTypeEnd - index);
                }
                itemTypeStart = 0;
                itemTypeEnd = 0;
                totalCount = 0;
        ...
        public void ScrollToNextPage(float speed, LoopScrollManager.ScrollPageEndCallBack callback)
        {
            float offset = directionSign == -1 ? -viewRect.rect.size.y : viewRect.rect.size.x;
            StopAllCoroutines();
            StartCoroutine(ScrollToCellCoroutine(offset, speed, callback));
        }
        public void ScrollToPrewPage(float speed, LoopScrollManager.ScrollPageEndCallBack callback)
        {
            float offset = directionSign == -1 ? viewRect.rect.size.y : -viewRect.rect.size.x;
            StopAllCoroutines();
            StartCoroutine(ScrollToCellCoroutine(offset, speed, callback));
        }
        IEnumerator ScrollToCellCoroutine(float offset, float speed, LoopScrollManager.ScrollPageEndCallBack callback)
        {
            ...
            while (needMoving)
            {
                ...
                if (move != 0)
                {
                    Vector2 offsetV = GetVector(move);
                    content.anchoredPosition += offsetV;
                    m_PrevPosition += offsetV;
                    m_ContentStartPosition += offsetV;
                    offset -= move;
                    UpdateBounds(true);
                }
            }
        ...
        public void RefreshCells(bool bChageTotal = true, bool bSetRefresh = true)
        {
            if (Application.isPlaying && this.isActiveAndEnabled)
            {
                ...
                for (int i = 0; i < content.childCount; i++)
                {
                    ...
                    {
                        m_despawnCallBack(content.GetChild(i), i);
                        RectTransform newItem = InstantiateNextItem(itemTypeEnd);
                        newItem.SetSiblingIndex(i);
                        //m_callBack(content.GetChild(i).gameObject, itemTypeEnd, bSetRefresh);
                        itemTypeEnd++;
                    }
                    else
                    {
        ...
        public void RefreshCells(int index, int count)
        {
            if (Application.isPlaying && this.isActiveAndEnabled)
            {
                int _start = index;
                int _end = index + count;
                itemTypeEnd = itemTypeStart;
                // recycle items if we can
                for (int i = 0; i < content.childCount; i++)
                {
                    if (itemTypeEnd >= _start && itemTypeEnd < _end)
                    {
                        if (itemTypeEnd < totalCount)
                        {
                            m_callBack(content.GetChild(i).gameObject, itemTypeEnd, true);
                            itemTypeEnd++;
                        }
                        else
                        {
        ...
        public void AddCells(int index, int count)
        {
            ...
            int i = 0;
            while (sizeToFill > sizeFilled || itemTypeStart + i < itemTypeEnd)
            {
                float size = 0;
                if (itemTypeStart + i >= totalCount)
                {
                    break;
                }
                if (itemTypeStart + i < index || itemTypeStart + i >= index + count)
                {
                    if (i < content.childCount)
                    {
                        size = GetSize(content.GetChild(i) as RectTransform);
                    }
                    else
                    {
                        size = reverseDirection ? NewItemAtEnd() : NewItemAtStart();
                    }
                }
                else
                {
                    size = NewItemAtIndex(itemTypeStart + i);
                }
                sizeFilled += Math.Max(size, 0);
                i++;
            }
            UpdateBounds();
        }
        public void DelCells(int index, int count)
        {
            if (Application.isPlaying && this.isActiveAndEnabled)
            {
                StopAllCoroutines();
        ...
        public void RefillCellsFromEnd(int offset = 0)
        {
            ...
            {
                m_despawnCallBack(content.GetChild(i), itemTypeStart + i);
            }
        ...
        public void RefillCells(int offset = 0)
        {
            ...
            for (int i = m_Content.childCount - 1; i >= 0; i--)
            {
                itemSize = Mathf.Max(GetSize(content.GetChild(i) as RectTransform), itemSize);
                m_despawnCallBack(content.GetChild(i), i);
            }
            ...
            bool bEnd = offset == totalCount - 1 && offset != 0;
            if (itemSize > 0 && offset > 0)
            {
                int endIdx = totalCount - Mathf.CeilToInt(sizeToFill / itemSize);
                offset = Mathf.Min(offset, endIdx);
                if (offset <= 0)
                {
                    offset = 0;
                    bEnd = false;
            ...
            m_Content.anchoredPosition = pos;
            m_content_anchoredpos = content.anchoredPosition;
        ...
        protected float DeleteItemAtStart()
        {
            ...
            for (int i = 0; i < contentConstraintCount; i++)
            {
                ...
                if (m_despawnCallBack != null)
                    m_despawnCallBack(oldItem, itemTypeStart);
        ...
        protected float NewItemAtIndex(int index)
        {
            float size = 0;
            // issue 4: fill lines to end first
            int count = contentConstraintCount - (content.childCount % contentConstraintCount);
            for (int i = 0; i < count; i++)
            {
                RectTransform newItem = InstantiateNextItem(index);
                newItem.SetSiblingIndex(index - itemTypeStart);
                size = Mathf.Max(GetSize(newItem), size);
                itemTypeEnd++;
            }
        ...
        {
            //Debug.LogFormat("chat content scroll InstantiateNextItem index:{0}",itemIdx);
            RectTransform nextItem = m_spawnCallBack(itemIdx).GetComponent<RectTransform>();
            nextItem.transform.SetParent(content, false);
            nextItem.gameObject.SetActive(true);
            m_callBack(nextItem.gameObject, itemIdx);
            return nextItem;
        }
        //==========LoopScrollRect==========
        ...
        protected virtual void SetContentAnchoredPosition(Vector2 position)
        {
            ...
            if (position != m_Content.anchoredPosition)
            {
                if (m_Horizontal)
                {
                    deltaX += position.x - m_Content.anchoredPosition.x;
                    if (m_moveCallback != null)
                    {
                        m_moveCallback(deltaX);
                    }
                }

                if (m_Vertical)
                {
                    deltaY += position.y - m_Content.anchoredPosition.y;
                    if (m_moveCallback != null)
                    {
                        m_moveCallback(deltaY);
                    }
                }
        ...
        protected virtual void LateUpdate()
        {
            ...
            if (m_ViewBounds != m_PrevViewBounds || m_ContentBounds != m_PrevContentBounds || m_Content.anchoredPosition != m_PrevPosition)
            {
                ...
                if (vertical && horizontal)
                {
                    if (normalizedPosition.x >= 1 - element_offset.x && normalizedPosition.y >= 1 - element_offset.y)
                    {
                        if (onScrollBottom != null)
                        {
                            onScrollBottom.Invoke(true);
                ...
                {
                    if (normalizedPosition.y >= 1 - element_offset.y)
                    {
                        if (onScrollBottom != null)
                        {
                            onScrollBottom.Invoke(true);
                ...
                {
                    if (normalizedPosition.x >= 1 - element_offset.x)
                    {
                        if (onScrollBottom != null)
                        {
                            onScrollBottom.Invoke(true);
