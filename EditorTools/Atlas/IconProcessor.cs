...
public class IconProcessor
{
    ...
    public static string SINGLE_ROOT = $"{GameConfig.PackUGUIRootPath}/Image";
    public static string MUTLIPLE_ROOT = $"{GameConfig.PackUGUIRootPath}/Image";
    public static string SINGLE_OUT_ROOT = $"{GameConfig.PackUGUIRootPath}/Atlas";
    public static string MUTLIPLE_OUT_ROOT =  $"{GameConfig.PackUGUIRootPath}/Atlas";

    [MenuItem("Assets/MakeIconAtlas", false, 102)]
    public static void Main()
    {
        //主方法,MakeIconAtlas进来执行的地方
    ...
    private static void EnterByFolder(string path)
    {
        ...
        else if (path.Contains(MUTLIPLE_ROOT) == true)
        {
            ...
            {
                //路径string长度大于多图根路径长度,说明选择了一个多图文件夹路径
                PackFolderMode(path); //按多图路径打包
            }
            else
            {
                //处理选中了多图文件夹根目录的情况,把根目录包含的子文件夹都进行打包
    ...
    private static string[] GetAssetPaths(string folderPath)
    {
        ...
        string[] result = Directory.GetFiles(folderPath, "*.*", SearchOption.TopDirectoryOnly)
            .Where<string>(s => s.Contains(".meta") == false).ToArray<string>();
        for (int i = 0; i < result.Length; i++)
        {
            //通过过滤的文件列表,此时只包含图片资源
            result[i] = result[i].Replace(@"\", @"/"); //把所有的'\'替换成'/'
    ...
    {
        var al = path.Replace(SINGLE_ROOT, SINGLE_OUT_ROOT);
    ...
    private static void CreateSingleModeAtlas(string path, string atlasPath)
    {
        ...
        if (importer == null)
        {
            Debug.LogError("发现不是图片的资源, 资源路径 = " + atlasPath);
            return;
        } //不是图片资源就报错,找不到TextureImporter
    ...
    private static void CreateFolderModeAtlas(string[] paths, string atlasPath)
    {
        ...
        Rect[]
            rects = atlas.PackTextures(textures, 0, ATLAS_MAX_SIZE,
                false); //textures要打包到图集的纹理的数组,0 纹理间的像素填充,ATLAS_MAX_SIZE 纹理最大尺寸,false,不可读?false
